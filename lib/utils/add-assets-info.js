...
const ASSETS_DATA = {
  'darwin-x64': {
    install: [
      ...
      path.join('mac', '{productName}-{version}.pkg')
    ],
    metaFile: null,
    update: [
...
function resolveAssetPath(assetsPath, assetMask, options, exception = true) {
  ...
  const fileName = assetMask
    .replace('{name}', options.alternatePackageName || json.name)
    .replace('{productName}', options.alternateProductName || json.productName || json.name)
